...
function Pipe:init(orientation, y)
    self.x = Config.defaults.VIRTUAL_WIDTH + 64
    self.y = y

    self.width = Config.pipe.PIPE_WIDTH
    self.height = Config.pipe.PIPE_HEIGHT
...
function Pipe:render()
    love.graphics.draw(PIPE_IMAGE, self.x, 
        (self.orientation == 'top' and self.y + Config.pipe.PIPE_HEIGHT or self.y), 
